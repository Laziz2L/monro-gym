<template>
  <div class="add-form">
    <h6>
      {{ coachName == "no" ? "Записаться без тренера" : "Добавить тренировку" }}
    </h6>
    <br />
    <form @submit.prevent="store">
      ...
      <button type="submit" class="btn btn-success">
        {{ coachName == "no" ? "Записаться" : "Добавить" }}
      </button>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    store() {
      ...
      request.stopTime =
        stopHour + request.startTime.substring(2, request.startTime.length);
